...
import ArticleLayout from '@/components/layouts/ArticleLayout'; // ArticleLayoutをインポート

// このページが受け取るパラメータの型定義
interface ArticlePageProps {
  params: {
    id: string; // URLの [id] の部分（例: "1", "2" など）がここに入る
  };
}
...
export default function ArticlePage({ params }: ArticlePageProps) {
  ...
  if (articleId === "1") { // IDが"1"の場合のコンテンツ
    title = "2025年最新！おすすめオンラインカジノランキングTOP10";
    content = (
      <>
        <Image
          src="/images/articles/ranking_2025.jpg" // ★ここに記事のメイン画像パスを設定（publicフォルダ内から）★
          alt="2025年最新オンラインカジノランキング"
          width={1200} // 画像のオリジナルのアスペクト比に合わせて調整
          height={675} // 画像のオリジナルのアスペクト比に合わせて調整
  ...
  } else if (articleId === "2") { // IDが"2"の場合のコンテンツ（「初心者必見！オンラインカジノの始め方完全ガイド」）
    title = "初心者必見！オンラインカジノの始め方完全ガイド";
    content = (
      <>
        <Image
          src="/images/articles/beginners_guide.jpg" // ★ここに記事のメイン画像パスを設定（publicフォルダ内から）★
          alt="オンラインカジノの始め方"
          width={1200}
          height={675}
          className="rounded-lg mb-8 w-full h-auto"
  ...
  } else if (articleId === "3") { // IDが"3"の場合のコンテンツ（「勝率アップ！バカラの必勝法と攻略テクニック」）
    title = "勝率アップ！バカラの必勝法と攻略テクニック";
    content = (
      <>
        <Image
          src="/images/articles/baccarat_strategy.jpg" // ★ここに記事のメイン画像パスを設定（publicフォルダ内から）★
          alt="バカラ必勝法"
          width={1200}
          height={675}
          className="rounded-lg mb-8 w-full h-auto"
